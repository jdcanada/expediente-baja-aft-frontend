...
export class PdfGeneratorService {
  ...
  // ============================================
  // GENERAR INFORME RESUMEN PDF
  // ============================================

  generateInformeResumenPDF(expediente: ExpedienteFormulario, medios: AFTRegistro[]): Observable<Blob> {
    return new Observable(observer => {
      (async () => {
        try {
          const pdfMakeModule = await import('pdfmake/build/pdfmake');
          ...
          const docDefinition = this.buildInformeResumenDocDefinition(expediente, medios);
          const pdfDocGenerator = pdfMakeModule.default.createPdf(docDefinition);
  ...
  private buildInformeResumenDocDefinition(expediente: ExpedienteFormulario, medios: AFTRegistro[]): any {
    const areas = this.obtenerAreasUnicas(medios);
    ...
    return {
      content: [
        ...
        { text: [{ text: 'Fecha: ', bold: true }, this.formatearFecha(expediente.fecha_creacion)] },
        ...
        {
          text: [
            'Por medio este solicitamos a la Dirección de Contabilidad que se tramite la baja de los siguientes ',
            { text: `${medios.length}`, bold: true },
        ...
        { text: 'ANEXO I. Modelo de propuesta a baja de los AFT que se encuentran en mal estado o deteriorados totalmente.', bold: true, margin: [0, 0, 0, 10] },
        {
          table: {
            ...
            body: [
              ...
              ...medios.map((r, i) => [i + 1, r.mb, r.aft, r.area])
            ]
          },
        ...
        { text: 'ANEXO III. Resumen de los Dictámenes Técnicos', bold: true, margin: [0, 0, 0, 10] },
        {
          table: {
            ...
            body: [
              ...
              ...medios.map((r, i) => [i + 1, r.mb, r.aft, r.area, r.noDictamen || '', this.formatearFecha(expediente.fecha_creacion)])
            ]
          },
          layout: 'lightHorizontalLines',
          margin: [0, 0, 0, 10]
        ...
        },
        {
          table: {
            ...
            body: [
              ...
              ...medios.map((r, i) => [i + 1, r.mb, r.aft, r.area, r.destinoFinal || ''])
            ]
          },
          layout: 'lightHorizontalLines',
          margin: [0, 0, 0, 10]
  ...
  private obtenerAreasUnicas(medios: AFTRegistro[]): string[] {
    if (!medios) return [];
    const areas = medios.map(r => r.area);
